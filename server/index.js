...
await  db.all(sqlString1,[`${givenDate}`,`${nextDate}`,"typeA","typeB","Discoloration","Foreign Particles","Scratches","Others","Crack"],async(err,rows)=>{
    if(err){
      console.log(err)
      return
    }
  ...
  await db.all(sqlString,[`${givenDate}`,`${nextDate}`,"typeA","typeB","Discoloration","Foreign Particles","Scratches","Others","Crack"],async(err,rows)=>{
   ...
   await db.all( barDataQuary,[`${givenDate}`,`${nextDate}`,"typeA","typeB","Discoloration","Foreign Particles","Scratches","Others","Crack"],async(err,rows)=>{
    if(err){
      console.log(err)
    }
    await chartData.push(rows)
...
app.post("/data/filter",async (req, res) => {
 ...
 let {fromDate,toDate} = req.body
  ...
  if(filters.includes("Crack")){
    bottletypes.push('Crack')
  }else{
    bottletypes.push("")
  }
